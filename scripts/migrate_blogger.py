#!/usr/bin/env python3
import html, re, sys, urllib.request, xml.etree.ElementTree as ET
...
def section(labels,title):
    ...
    if any(k in x for k in ['nutri','alimenta','dieta','suplement','creatina','vitamina','proteína','proteina']): return 'Nutrição'
    if any(k in x for k in ['atividade física','atividade fisica','exerc','caminh','corrida','movimento','academia']): return 'Movimento'
    if any(k in x for k in ['mente','ansiedade','depress','tdah','pânico','panico','sono','estresse']): return 'Mente'
    if any(k in x for k in ['longevid','envelhe','depois dos 40','menopausa']): return 'Longevidade'
    return 'Saúde'
...
def main():
    req=urllib.request.Request(FEED,headers={'User-Agent':'VitaCerta-Migrator/1.0'})
    ...
    Path('sitemap.xml').write_text('<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"><url><loc>https://vitacerta.com.br/</loc></url>'+urls+'</urlset>',encoding='utf-8')
