...
INSTITUTIONAL_URLS=['sobre/','contato/','politica-de-privacidade/','termos-de-uso/']
...
def sec_id(sec): return {'Saúde':'saude','Nutrição':'nutricao','Movimento':'movimento','Mente':'mente','Longevidade':'longevidade'}[sec]

def page(title,body,sec,published,slug):
    desc=re.sub('<[^>]+>',' ',body); desc=re.sub(r'\s+',' ',html.unescape(desc)).strip()[:158]
...
def inject_home(posts):
    ...
    src=path.read_text(encoding='utf-8'); marker='<!-- VC_ARTIGOS_HOME -->'
    if marker in src: src=src.split(marker)[0] + '</body></html>'
    latest=posts[:8]
    rows=''.join(f'''<article class="vc-row"><div class="vc-num">{i:02d}</div><div><div class="vc-meta"><span class="vc-tag">{esc(sec)}</span><span>{esc(pub[:10])}</span></div><h3><a href="/conteudos/{slug}/">{esc(title)}</a></h3><a class="vc-read" href="/conteudos/{slug}/">Ler artigo →</a></div></article>''' for i,(pub,title,slug,sec) in enumerate(latest,1))
    ...
    patch=f'''{marker}<style>#conteudos{{display:block!important;max-width:1240px!important;margin-left:auto!important;margin-right:auto!important;padding:40px 20px 56px!important}}.vc-home-head{{display:flex;align-items:end;justify-content:space-between;gap:20px;padding-bottom:16px;border-bottom:1px solid #E7E5E4}}.vc-kicker{{font-family:Inter,sans-serif;font-size:11px;font-weight:600;letter-spacing:.14em;text-transform:uppercase;color:#78716C}}.vc-home-head h2{{font-family:'Cormorant Garamond',serif;font-size:30px;line-height:1.05;margin:5px 0 0;color:#0F172A}}.vc-all-top,.vc-read{{font-family:Inter,sans-serif;color:#0F172A;text-decoration:underline;text-decoration-color:#8A9A8E;text-underline-offset:4px}}.vc-all-top{{font-size:13px;white-space:nowrap}}.vc-chips{{display:flex;flex-wrap:wrap;gap:8px;margin:20px 0 8px}}.vc-chips a{{font-family:Inter,sans-serif;font-size:12px;text-decoration:none;color:#44403C;background:#fff;border:1px solid #E7E5E4;border-radius:999px;padding:7px 11px}}.vc-list{{border-top:1px solid #E7E5E4}}.vc-row{{display:grid;grid-template-columns:48px 1fr;gap:20px;padding:24px 0;border-bottom:1px solid #E7E5E4}}.vc-num{{font-family:'Cormorant Garamond',serif;font-weight:700;font-size:28px;color:#D6D3D1}}.vc-meta{{display:flex;flex-wrap:wrap;align-items:center;gap:9px;font-family:Inter,sans-serif;font-size:11px;color:#78716C;margin-bottom:8px}}.vc-tag{{background:#0F172A;color:#fff;border-radius:999px;padding:4px 8px;font-weight:600;letter-spacing:.08em;text-transform:uppercase}}.vc-row h3{{font-family:'Cormorant Garamond',serif;font-size:24px;line-height:1.15;margin:0 0 10px;color:#0F172A}}.vc-row h3 a{{color:inherit;text-decoration:none}}.vc-row h3 a:hover{{text-decoration:underline;text-decoration-color:#8A9A8E;text-underline-offset:5px}}.vc-read{{font-size:12px}}@media(max-width:700px){{.vc-home-head{{align-items:start;flex-direction:column}}.vc-row{{grid-template-columns:38px 1fr;gap:10px}}.vc-row h3{{font-size:21px}}}}</style><script>(function(){{var html={home!r};var tries=0;function patchHome(){{var main=document.getElementById('conteudos');if(!main){{if(tries++<80)setTimeout(patchHome,50);return;}}main.innerHTML=html;main.setAttribute('data-vc-live','1');var map={{'Saúde':'saude','Nutrição':'nutricao','Movimento':'movimento','Mente':'mente','Longevidade':'longevidade'}};document.querySelectorAll('a[href="#conteudos"]').forEach(function(a){{var t=(a.textContent||'').trim();a.href=map[t]?'/conteudos/#'+map[t]:'/conteudos/';}});}}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',patchHome);else patchHome();}})();</script>'''
    src=src.replace('</body>',patch+'</body>',1) if '</body>' in src else src+patch
    path.write_text(src,encoding='utf-8')

def build_sitemap(posts):
    base='https://vitacerta.com.br/'
    fixed=['', 'conteudos/'] + INSTITUTIONAL_URLS
    entries=''.join(f'<url><loc>{base}{p}</loc></url>' for p in fixed)
    entries+=''.join(f'<url><loc>{base}conteudos/{slug}/</loc></url>' for _,_,slug,_ in posts)
    return '<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">'+entries+'</urlset>'

def main():
    req=urllib.request.Request(FEED,headers={'User-Agent':'VitaCerta-Migrator/1.4'})
    raw=urllib.request.urlopen(req,timeout=30).read(); root=ET.fromstring(raw); OUT.mkdir(exist_ok=True)
    seen=set(); posts=[]
    for e in root.findall('a:entry',NS):
        title=(e.findtext('a:title',default='',namespaces=NS) or '').strip(); body=(e.findtext('a:content',default='',namespaces=NS) or '').strip()
        if not title or not body: continue
        cats=[c.attrib.get('term','') for c in e.findall('a:category',NS)]; kind=[c for c in cats if c.startswith('http://schemas.google.com/blogger/2008/kind#')]
        if kind and not any(c.endswith('#post') for c in kind): continue
        key=re.sub(r'\s+',' ',title).casefold()
        if key in seen: continue
        seen.add(key); labels=[c for c in cats if not c.startswith('http://schemas.google.com/')]
        pub=e.findtext('a:published',default='',namespaces=NS) or ''; slug=slugify(title); sec=section(labels,title)
        d=OUT/slug; d.mkdir(parents=True,exist_ok=True); (d/'index.html').write_text(page(title,body,sec,pub,slug),encoding='utf-8'); posts.append((pub,title,slug,sec))
    posts.sort(reverse=True); (OUT/'index.html').write_text(build_catalog(posts),encoding='utf-8'); inject_home(posts)
    Path('sitemap.xml').write_text(build_sitemap(posts),encoding='utf-8')
    print(f'Publicados {len(posts)} artigos, home integrada e sitemap atualizado com páginas institucionais.')
